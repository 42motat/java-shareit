...
public class BookingController {
    ...
    @PostMapping
    public BookingDto create(@RequestHeader(CUSTOM_USER_ID_HEADER) Long userId,
    ...
    @GetMapping("/{id}")
    public BookingDto getById(@RequestHeader(CUSTOM_USER_ID_HEADER) Long userId,
                              @PathVariable Long id) {
        return bookingService.getById(userId, id);
    }

    @GetMapping
    public Collection<BookingDto> getBookingsForBooker(@RequestHeader(CUSTOM_USER_ID_HEADER) Long userId,
                                                     @RequestParam(defaultValue = "ALL") BookingState bookingState) {
        return bookingService.getAllBookingsByBooker(userId, bookingState);
    }

    @GetMapping("/owner")
    public Collection<BookingDto> getBookingsForOwner(@RequestHeader(CUSTOM_USER_ID_HEADER) Long userId,
                                                     @RequestParam(defaultValue = "ALL") BookingState bookingState) {
